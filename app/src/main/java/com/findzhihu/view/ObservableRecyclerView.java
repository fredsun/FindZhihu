...
public class ObservableRecyclerView extends RecyclerView implements Scrollable {
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (getChildCount() > 0) {
            ...
            if (firstVisibleChild != null) {
                    ...
                    if (firstVisiblePosition - mPrevFirstVisiblePosition != 1) {
                        for (int i = firstVisiblePosition - 1; i > mPrevFirstVisiblePosition; i--) {
                            if (0 < mChildrenHeights.indexOfKey(i)) {
                                ...
                                // Approximate each
                                // item's height to the first visible child.
                ...
                dispatchOnScrollChanged(mScrollY, mFirstScroll, mDragging, l, t, oldl, oldt);
                if (mFirstScroll) {
                    mFirstScroll = false;
                }
    ...
    private void dispatchOnScrollChanged(int scrollY, boolean firstScroll, boolean dragging, int a, int b, int oldA, int oldB) {
        if (mCallbacks != null) {
            mCallbacks.onScrollChanged(scrollY, firstScroll, dragging);
            mCallbacks.onScrollChanged(scrollY, firstScroll, dragging, a, b, oldA, oldB);
        }
        if (mCallbackCollection != null) {
            for (int i = 0; i < mCallbackCollection.size(); i++) {
                ObservableScrollViewCallbacks callbacks = mCallbackCollection.get(i);
                callbacks.onScrollChanged(scrollY, firstScroll, dragging);
                callbacks.onScrollChanged(scrollY, firstScroll, dragging, a, b, oldA, oldB);
            }
        }
    }
